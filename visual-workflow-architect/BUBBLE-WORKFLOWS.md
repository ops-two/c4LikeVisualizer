...
**Trigger:** Element Event - `container_reordered` OR `sequence_reordered`
**Condition:** `pending_reorder` is not empty

**Actions:**
1. **Extract Entity Type:**
   ```
   Search & Replace in pending_reorder
   Find: "entityType":"([^"]*)"
   Replace with: Entity Type: $1
   ```

2. **Process Reorder Data (Custom Code or API Workflow):**
   ```javascript
   // This requires a more complex workflow or API endpoint
   // to process the array of reorder data
   const reorderData = JSON.parse(pending_reorder).reorderData;
   reorderData.forEach(item => {
     // Update order_index_number for each entity
   });
   ```
...
| `pending_reorder` | Text | JSON payload for reordering operations |

## Event Names

The plugin will trigger these events:

- `container_updated` - When a container name is edited
- `sequence_updated` - When a sequence label is edited
- `container_to_be_added` - When a new container is created
- `sequence_to_be_added` - When a new sequence is created
- `container_to_be_deleted` - When a container is deleted
- `sequence_to_be_deleted` - When a sequence is deleted
- `container_reordered` - When containers are reordered
- `sequence_reordered` - When sequences are reordered
