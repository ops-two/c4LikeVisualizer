...
window.WorkflowArchitectRenderer = {
  ...
  init: function (containerId) {
    console.log(
      "WorkflowArchitectRenderer: Initializing for container:",
      containerId
    );
  ...
  initializeInteractivity: function () {
    console.log(
      "WorkflowArchitectRenderer: Initializing interactivity systems"
    );

    // Initialize inline edit system
    if (
      window.SequenceDiagramInlineEdit &&
      !window.SequenceDiagramInlineEdit.isInitialized
    ) {
    ...
    if (
      ...
      !window.SequenceDiagramEventBridge.isInitialized &&
      window.bubbleInstance
    ) {
    ...
    if (
      ...
      !window.SequenceDiagramDataStore.isInitialized
    ) {
      // Data will be initialized from bubble-compatible-update.js
      console.log(
        "WorkflowArchitectRenderer: Data store will be initialized by update handler"
      );
    }
  },
  ...
  renderSequenceDiagramContent: function (data, containerId) {
    ...
    if (container.hasAttribute("data-rendered")) {
      console.log(
        "WorkflowArchitectRenderer: Container already rendered, skipping"
      );
      return;
    }
  ...
  transformToSequenceDiagram: function (data) {
      ...
      data.sequences.forEach((sequence, index) => {
        ...
        for (const field of possibleFromFields) {
          if (sequence[field]) {
            ...
            console.log(
              `DEBUG: Found from container ID in field '${field}':`,
              fromContainerId
            );
            break;
          }
        ...
        for (const field of possibleToFields) {
          if (sequence[field]) {
            ...
            console.log(
              `DEBUG: Found to container ID in field '${field}':`,
              toContainerId
            );
            break;
          }
        ...
        if (!fromContainerId || !toContainerId) {
          console.log(
            "DEBUG: Missing container IDs - using fallback sequential mapping"
          );
          ...
          if (totalContainers >= 2) {
            ...
            console.log(
              `DEBUG: Added fallback message from ${fromIndex} to ${toIndex} (${data.containers[fromIndex].name_text} -> ${data.containers[toIndex].name_text})`
            );
          }
          return;
        }
        ...
        const possibleContainerIdFields = [
          "container_id",
          "id",
          "_id",
          "unique_id",
        ...
        for (const field of possibleContainerIdFields) {
          if (fromIndex === -1) {
            fromIndex = data.containers.findIndex(
              (c) => c[field] === fromContainerId
            );
            if (fromIndex !== -1)
              console.log(
                `DEBUG: Found from container using field '${field}' at index:`,
                fromIndex
              );
          }
          if (toIndex === -1) {
            toIndex = data.containers.findIndex(
              (c) => c[field] === toContainerId
            );
            if (toIndex !== -1)
              console.log(
                `DEBUG: Found to container using field '${field}' at index:`,
                toIndex
              );
          }
        }
        ...
        console.log(
          `DEBUG: Final indices - from: ${fromIndex}, to: ${toIndex}`
        );

        if (fromIndex !== -1 && toIndex !== -1) {
          messages.push({
            label:
              sequence.label_text || sequence.name_text || "Untitled Sequence",
            from: fromIndex,
            to: toIndex,
            dashed: sequence.is_dashed_boolean || false,
          });
    ...
    console.log(
      "WorkflowArchitectRenderer: Transformed to",
      actors.length,
      "actors and",
      messages.length,
      "messages"
  ...
  createSequenceDiagramComponent: function (actors, messages) {
    ...
    const ActivationBox = ({ actorIndex, yPos, color }) => {
      ...
      const positionX =
        actorIndex * (100 / actorsCount) + 100 / (actorsCount * 2);
      const style = {
        top: `${yPos}px`,
        left: `${positionX}%`,
...
console.log(
  "DEBUG: sequence-renderer.js script loaded successfully. WorkflowArchitectRenderer object created:",
